...
$QUERY = $dbh->prepare("select StartCountry, tblCountries.Primary, count(StartCountry) AS Kolikrat from tblRuns, tblCountries where StartCountry <> 'NULL' and StartCountry = tblCountries.Country group by StartCountry;");
$QUERY->execute() or die "Nepovedlo se nacist statistiky 3!";
while (@DATA =  $QUERY->fetchrow_array) {
	push @SQL, 'Zeme.'.$DATA[0].'.'.$DATA[1],$DATA[2];
}
...
$QUERY = $dbh->prepare("select ROUND(SUM(Distance/1000),2) AS ThisMonthDistance, Month(CURDATE()) AS Mesic from tblRuns where Year(convert_tz(Date, 'UTC','CET')) = Year(CURDATE()) and Month(convert_tz(Date, 'UTC','CET')) = Month(CURDATE());");
...
$QUERY = $dbh->prepare("select ROUND(SUM(Distance/1000),2) AS LastMonthDistance, MONTH(DATE_ADD(CURDATE(), INTERVAL -1 MONTH)) AS MinulyMesic from tblRuns where Year(convert_tz(Date, 'UTC','CET')) = Year(DATE_ADD(CURDATE(), INTERVAL -1 MONTH)) and MONTHNAME(convert_tz(Date, 'UTC','CET')) = MONTHNAME(DATE_ADD(CURDATE(), INTERVAL -1 MONTH));");
...
$QUERY->execute() or die "Nepovedlo se nacist statistiky 7!";
$QUERY = $dbh->prepare("SELECT  \@RowNum := IF(\@NextDate = Date(tblRuns.Date), \@RowNum + 1, 1) AS RowNumber, DATE_FORMAT((tblRuns.Date), '%d. %m. %Y') AS Datum, tblRuns.`Primary`, DATE_FORMAT(DATE_ADD(Date(tblRuns.Date), INTERVAL (\@RowNum*(-1))+1 DAY), '%d. %m. %Y') AS StartDate, \@NextDate := DATE_ADD(Date(tblRuns.Date), INTERVAL 1 DAY) AS NextDate, DATE_ADD(Date(tblRuns.Date), INTERVAL (\@RowNum*(-1))+1 DAY) AS StartDate2 FROM tblRuns ORDER BY RowNumber DESC LIMIT 1;");
$QUERY->execute() or die "Nepovedlo se nacist statistiky 7!";
while (@DATA =  $QUERY->fetchrow_array) {
	...
	my $QUERY2 = $dbh->prepare("select tblRuns.`Primary` from tblRuns where Date(tblRuns.Date) = \'$DATA[5]\';");
...
while (@DATA =  $QUERY->fetchrow_array) {
	push @SQL, $DATA[0].'.KorDistance',$DATA[1];
	push @SQL, $DATA[0].'.KorSecForPace',$DATA[2];
}
